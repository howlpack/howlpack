...
const howlPreviewConfig = new pulumi.Config("howl-preview");
const howlPreviewPackageName = "howlpack--package-howl-preview";

const domain = howlPreviewConfig.require("API_DOMAIN");

const contentBucket = new aws.s3.BucketV2(
  howlPreviewPackageName + "-contentBucket",
  {
    bucket: domain,
  }
);
...
const bucketOwnershipControls = new aws.s3.BucketOwnershipControls(
  howlPreviewPackageName + "bucketOwnershipControls",
  {
...
const bucketPublicAccessBlock = new aws.s3.BucketPublicAccessBlock(
  howlPreviewPackageName + "bucketPublicAccessBlock",
  {
    bucket: contentBucket.id,
...
new aws.s3.BucketAclV2(
  howlPreviewPackageName + "bucketAclV2",
  {
    bucket: contentBucket.id,
    acl: "public-read",
  },
...
const contentBucketWebsiteConf = new aws.s3.BucketWebsiteConfigurationV2(
  howlPreviewPackageName + "-contentBucketWebsiteConf",
  {
    bucket: contentBucket.id,
    indexDocument: {
      suffix: "index.html",
...
const contentBucketPolicy = new aws.s3.BucketPolicy(
  howlPreviewPackageName + "-contentBucketPolicy",
  {
    bucket: contentBucket.id,
...
const awsUsEast1 = new aws.Provider(howlPreviewPackageName + "usEast1", {
  region: "us-east-1",
});

const certificate = new aws.acm.Certificate(
  howlPreviewPackageName + "-sslCert",
  {
    domainName: domain,
...
const hostedZoneId = aws.route53
  .getZone(
    { name: howlPreviewConfig.require("ROUTE53_ZONE_NAME") },
    { async: true }
  )
  .then((zone) => zone.zoneId);

const certificateValidationDomain = new aws.route53.Record(
  howlPreviewPackageName + `-${domain}-validation`,
  {
...
const subdomainCertificateValidationDomain = new aws.route53.Record(
  howlPreviewPackageName + `${domain}-validation2`,
  {
...
const certificateValidation = new aws.acm.CertificateValidation(
  howlPreviewPackageName + "-certificateValidation",
  {
...
const distribution = new aws.cloudfront.Distribution(
  howlPreviewPackageName + "-cdn",
  distributionArgs
);

new aws.route53.Record(howlPreviewPackageName + "-" + domain, {
  name: domain,
  zoneId: hostedZoneId,
  type: "A",
  aliases: [
...
new aws.route53.Record(howlPreviewPackageName + `-${domain}-www-alias`, {
  name: `www.${domain}`,
  zoneId: hostedZoneId,
  type: "A",
  aliases: [
