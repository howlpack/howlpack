...
const gateway2023Config = new pulumi.Config("gateway2023");
const gateway2023PackageName = "howlpack--package-2023";

const domain = gateway2023Config.require("API_DOMAIN");

const contentBucket = new aws.s3.BucketV2(
  gateway2023PackageName + "-contentBucket",
  {
    bucket: domain,
  }
);
...
const bucketOwnershipControls = new aws.s3.BucketOwnershipControls(
  gateway2023PackageName + "bucketOwnershipControls",
  {
...
const bucketPublicAccessBlock = new aws.s3.BucketPublicAccessBlock(
  gateway2023PackageName + "bucketPublicAccessBlock",
  {
    bucket: contentBucket.id,
...
new aws.s3.BucketAclV2(
  gateway2023PackageName + "bucketAclV2",
  {
    bucket: contentBucket.id,
    acl: "public-read",
  },
...
const contentBucketWebsiteConf = new aws.s3.BucketWebsiteConfigurationV2(
  gateway2023PackageName + "-contentBucketWebsiteConf",
  {
    bucket: contentBucket.id,
    indexDocument: {
      suffix: "index.html",
...
const contentBucketPolicy = new aws.s3.BucketPolicy(
  gateway2023PackageName + "-contentBucketPolicy",
  {
    bucket: contentBucket.id,
...
const awsUsEast1 = new aws.Provider(gateway2023PackageName + "usEast1", {
  region: "us-east-1",
});

const certificate = new aws.acm.Certificate(
  gateway2023PackageName + "-sslCert",
  {
    domainName: domain,
...
const hostedZoneId = aws.route53
  .getZone(
    { name: gateway2023Config.require("ROUTE53_ZONE_NAME") },
    { async: true }
  )
  .then((zone) => zone.zoneId);

const certificateValidationDomain = new aws.route53.Record(
  gateway2023PackageName + `-${domain}-validation`,
  {
...
const subdomainCertificateValidationDomain = new aws.route53.Record(
  gateway2023PackageName + `${domain}-validation2`,
  {
...
const certificateValidation = new aws.acm.CertificateValidation(
  gateway2023PackageName + "-certificateValidation",
  {
...
const distribution = new aws.cloudfront.Distribution(
  gateway2023PackageName + "-cdn",
  distributionArgs
);

new aws.route53.Record(gateway2023PackageName + "-" + domain, {
  name: domain,
  zoneId: hostedZoneId,
  type: "A",
  aliases: [
...
new aws.route53.Record(gateway2023PackageName + `-${domain}-www-alias`, {
  name: `www.${domain}`,
  zoneId: hostedZoneId,
  type: "A",
  aliases: [
